...
#    This script downloads RH forecasts for 12h (GMT) and creates .tsf files
...
output_file_path='./TimeSeries/'
site_file_path='./'
remote_variable='CMC_naefs-geps-raw_RH_TGL_2m_latlon0p5x0p5_'
variable_type='RH'
# max number of hours=384
maxhours=24
# max scenario count = 21
scenario_count=2
...
do
 wget -r -l1 --no-parent -nd -A '*RH_TGL*'$yest'*' $remote_location'/'$run_time'/'$hour'/'
done
...
# get GRIB file using wildcards to uniquely identify each hour
   ...
   for f in $grib_file_path$FILES'*P'$hour'*.grib2'
    ...
    # extract each line to a separate file
     for l in `seq 1 1 $scenario_count`
      do
...
#convert RH and adjust time to be MST for each scenario
 for index in ${!names[*]}
  do
   site=${names[$index]}
   for l in `seq 1 1 $scenario_count`
     do
       gawk -f $awk_script_path'Extract'$variable_type'.awk'  $temp_file_path$variable_type$index'scenario'$l'.tmp' > $output_file_path$site'_'$variable_type'_Forecast_scenario'$l'.tsf'
     done
 done
