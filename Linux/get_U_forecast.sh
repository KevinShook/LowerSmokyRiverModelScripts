...
#    This script downloads U (wind vector) forecasts for 12h (GMT) and creates .tsf files
...
site_file_path='./'
remote_variable='CMC_naefs-geps-raw_UGRD_TGL_10m_latlon0p5x0p5_'
variable_type='U'
...
#convert wind speed and adjust time to be MST for each scenario
 for index in ${!names[*]}
  do
   site=${names[$index]}
   for l in `seq 1 1 $scenario_count`
