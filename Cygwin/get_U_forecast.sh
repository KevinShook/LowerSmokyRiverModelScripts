...
do
 echo  'wget' -r -l1 --no-parent -nd -A $remote_variable$yest'*' $remote_location'/'$run_time'/'$hour
...
#adjust time to be MST for each scenario, and output in WISKI model format
 for index in ${!names[*]}
  do
   site=${names[$index]}
   for l in `seq 1 1 1`
